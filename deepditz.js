...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Weight {
        ...
        weight() {
            return ((Math.random() - .5) * 2)/(Math.sqrt(this.whack)) //this strange thing here (whack) sets the starting weight absolute value to be low, and seems to help with training, but I'm not even sure
        }
        setChange(num) {
            this.change = num
        }
    ...
    class Perceptron {
        constructor(inputs) {
            this.inputs = inputs
            this.bias = (Math.random() - .5) / 10 //10 could be canged to inputs.length, but /10 seems to be good for what I have seen
        ...
        weight(link, whack) {
            let weight = new Weight(link, this, whack)  //make sure you know how many things you pass into this, if you get an error it's probably too many inputs for the network structure
    ...
    class Network {
        constructor(inputs, layerSetupArray, momentum = .01, learningRate = .02) {
        ...
        becomeNetworkFrom(network) { //using a js file with one variable can be good for this
            for (let t = 0; t < network.structure.length; t++) {
                for (let k = 0; k < network.structure[t].length; k++) {
                    ...
                    for (let w = 0; w < network.structure[t][k].weights.length; w++) {
                        this.structure[t][k].weights[w].setWeight(network.structure[t][k][w].valueOf())
                    }
                }
            }
        }
        calculateDeltasSigmoid(goals) { //back prop
            ...
            //decaying learning rate optional (1000) should be changed to your data set size
                ...
                if (t == -1) {
                    ...
                    for (let k = 0; k < this.net.structure[t].length; k++) {
                        ...
                        for (let g = 0; g < this.net.structure[t][k].weights.length; g++) {
                            ...
                            link.width = (2.44 * Math.sqrt(Math.abs(this.net.structure[t][k].weights[g].valueOf())))  // just something to visualize, feel free to mess with this 
                            if (this.net.structure[t][k].weights[g].valueOf() < 0) {
                                link.color = "#FF0000"
                            } else {
                                link.color = "#00FF00"
                            }
                            // //red-green colorblind mode, blue is positive, yellow is negative
                            // if (this.net.structure[t][k].weights[g].valueOf() < 0) {
                            //     link.color = "#ffff0066"
                            // } else {
                            //     link.color = "#0000ff66"
    ...
    let exampleInputs = []
    for (let t = 0; t < 4; t++) { // the 4 here, will be the number of inputs
        exampleInputs.push(new Data(Math.random()))
    }
    let exampleSetup = [4,4] //the last number in this array will be the number of outputs (4), the preceding numbers will be layers of nodes
    let exampleNet = new Network(exampleInputs, exampleSetup)
    exampleNet.compute(exampleInputs) //computing the network will fill the outputs array, and also will usually indicate problems if there are any




    function main() {
        for(let t = 0;t<1000;t++){ // this loop is a lot to ask if the network were any bigger, but for this example 1000 times every millisecond is ok.
            exampleInputs = [] //recycling this array for the example
            for (let t = 0; t < 4; t++) {  
                exampleInputs.push(new Data(Math.random()))
            }
            exampleNet.compute(exampleInputs)
    
            let exampleOutputs = []
            for (let t = 0; t < exampleInputs.length; t++) {  
                exampleOutputs.push(new Data(1-exampleInputs[t]))
            }
        
            exampleNet.calculateDeltasSigmoid(exampleOutputs)  // learn toward these outputs:inputs pairings
            exampleNet.adjustWeights() 
        }

        if(keysPressed[' ']){ //hold space to see the network computing
            canvas_context.clearRect(0, 0, canvas.width, canvas.height)  // refreshes the image
            let view = new Viewer(exampleNet)
            view.draw()
        }
    }
})
